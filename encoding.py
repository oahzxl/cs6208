...
class CentralityEncoding(nn.Module):
    ...
    """
    def __init__(
        self, num_heads, num_patches, patch_size, num_degree, hidden_dim
    ):
    ...
    def forward(self, x):
        x = einops.rearrange(x, "b c (h ph) (w pw) -> b (h w) (ph pw c)", h=self.num_patches, w=self.num_patches)

        x = self.patch_encoder(x)

        degree = torch.zeros([self.num_patches, self.num_patches], dtype=torch.int).cuda() + 2
        degree[0, :] -= 1
        degree[-1, :] -= 1
        degree[:, 0] -= 1
        degree[:, -1] -= 1
...
class SpatialEncoding(nn.Module):
    ...
    def __init__(
        ...
        self.num_heads = num_heads
        self.num_patches = num_patches
        self.spatial_bias = torch.zeros([self.num_patches ** 2 + 1, self.num_patches ** 2 + 1], dtype=torch.int).cuda()
...
class EdgeEncoding(nn.Module):
    ...
    def __init__(
        ...
        self.num_heads = num_heads
        self.num_patches = num_patches
        self.edge_bias = torch.zeros([self.num_patches ** 2 + 1, self.num_patches ** 2 + 1], dtype=torch.int).cuda()
        for i in range(self.edge_bias.shape[0] - 1):
            for j in range(self.edge_bias.shape[1] - 1):
                h = abs(i - j) // self.num_patches
                v = abs(i - j) % self.num_patches
                edge = torch.angle(torch.complex(torch.tensor(float(h)), torch.tensor(float(v))))
